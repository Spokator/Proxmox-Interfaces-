...
POST_INSTALL_WIZARD="${POST_INSTALL_WIZARD:-false}"
...
if [[ "$INSTALL_NOW" == "1" ]]; then
  ...
  pct exec "$CT_ID" -- bash -lc "chmod +x '${APP_DIR}/deploy/install.sh' && INSTALL_SYSTEM_UPGRADE='${INSTALL_SYSTEM_UPGRADE}' MANAGE_UFW='${MANAGE_UFW}' COMMUNITY_PROFILE='${DEPLOY_PROFILE}' POST_INSTALL_STACK_PROFILE='${DEPLOY_PROFILE}' POST_INSTALL_PLATFORM_SETUP='${POST_PLATFORM}' POST_INSTALL_PROFILE='auto' POST_INSTALL_WIZARD='${POST_INSTALL_WIZARD}' bash '${APP_DIR}/deploy/install.sh'"
fi
